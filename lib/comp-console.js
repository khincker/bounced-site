  ...
  class CompInstance {
    ...
    async _loadAudio() {
      ...
      const load = async (url, label) => {
        ...
        if (!buf.byteLength) throw new Error(`Empty response for ${label}`);
        return new Promise((resolve, reject) => {
          this.ctx.decodeAudioData(buf, resolve, (err) => {
      ...
      try {
        const [bufA, bufB] = await Promise.all([
          this.opts.trackA ? load(this.opts.trackA.url, this.opts.trackA.label || 'A').catch(e => { console.error('Track A load failed:', e); return null; }) : null,
          this.opts.trackB ? load(this.opts.trackB.url, this.opts.trackB.label || 'B').catch(e => { console.error('Track B load failed:', e); return null; }) : null,
        ]);

        this.bufferA = bufA;
        this.bufferB = bufB;

        // If all requested tracks failed to load, show error
        ...
        // Restore any state passed via mount options
