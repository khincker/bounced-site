...
const CompConsole = (() => {
  ...
  class CompInstance {
    constructor(container, opts) {
      ...
      this.beatGridVisible = false;
      this.driftMapVisible = false;
      this.markers = [];             // array of { sec, el }
      this.ghostMarker = null;       // { sec, el } or null

      // Zoom state
      this.isZoomed = false;
      this.zoomStart = 0;    // fraction 0-1 of full duration
      this.zoomEnd = 1;
    ...
    async _loadAudio() {
      // Only create context — don't resume yet (Safari blocks resume without user gesture)
      this._createContext();
      ...
      try {
        const [bufA, bufB] = await Promise.all([
          this.opts.trackA ? load(this.opts.trackA.url).catch(e => { console.warn('Track A load failed:', e); return null; }) : null,
          this.opts.trackB ? load(this.opts.trackB.url).catch(e => { console.warn('Track B load failed:', e); return null; }) : null,
        ]);
        ...
        // Extract peaks from aligned region only
        ...
        // Compute analysis data (defer beat detection to avoid blocking)
        ...
        if (this.opts.features.beatGrid && bufA) {
          // Run beat detection async via setTimeout to avoid blocking UI
          setTimeout(() => {
    ...
    _render() {
      ...
      // Loading overlay
      html += '<div class="comp-loading" data-el="loading"><div class="comp-loading-text" data-el="loadingText">Loading audio<span class="dot-anim"></span></div></div>';
      ...
      html += '<div class="comp-ab-toggle">';
      html += `<button class="comp-ab-btn active-a" data-el="abBtnA">A</button>`;
      html += `<button class="comp-ab-btn" data-el="abBtnB">B</button>`;
      html += '</div>';
      ...
      // Badge column
      html += '<div class="comp-badge-column">';
      html += '<div class="comp-badge-spacer"></div>';
      html += '<div class="comp-track-badge active-a" data-el="badgeA">A</div>';
      html += '<div class="comp-track-badge" data-el="badgeB">B</div>';
      html += '</div>';
      ...
      html += '</div></div>';
      html += '<div class="comp-track"><div class="comp-waveform-container" data-track="B">';
      ...
      this._drawRandomWaveform(this.els.canvasB, 'B');
    }
